...
sim_model = SentenceTransformer("/content/sup-simcse-roberta-large/")
tokenizer = AutoTokenizer.from_pretrained("/content/sup-simcse-roberta-large/")
sim_loss = losses.SoftmaxLoss(model=sim_model,sentence_embedding_dimension=sim_model.get_sentence_embedding_dimension(),num_labels=args.num_labels)
...
for train_index, test_index in skf.split(np.zeros(len(t)), t):
    ...
    sim_model.fit(train_objectives=[(train_dataloader, sim_loss)],epochs=epochs,warmup_steps=warmup_steps,show_progress_bar=True)
    ...
    test1_sim = sim_model.encode(test_data.Text1.values)
    test2_sim = sim_model.encode(test_data.Text2.values)
    test_diff_sim = np.subtract(test1_sim, test2_sim)
    testIN_sim = np.concatenate((test1_sim, test2_sim, test_diff_sim), axis=1)
    train1_sim = sim_model.encode(train_data.Text1.values)
    train2_sim = sim_model.encode(train_data.Text2.values)
    ...
    with open(args.output_path, "a") as f:
        f.write(classification_report(real, pred, labels=class_labels, digits = 6))
